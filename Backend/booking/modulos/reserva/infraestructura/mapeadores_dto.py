...
class MapeadorReservaDTO(Mapeador):
    ...
    def dto_a_entidad(self, dto: ReservaDTO) -> Reserva:
        return Reserva(
            id=uuid.UUID(dto.id),
            id_categoria=uuid.UUID(dto.id_categoria) if dto.id_categoria else None,
            ...
            fecha_check_in=date.fromisoformat(dto.fecha_check_in) if dto.fecha_check_in else None,
            fecha_check_out=date.fromisoformat(dto.fecha_check_out) if dto.fecha_check_out else None,
            ocupacion=Pax(
