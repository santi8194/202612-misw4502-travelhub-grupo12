...
void main() {
  ...
  group('SearchService - searchHotels', () {
    ...
    test('returns list of hotels and caches results on success (200)', () async {
      when(() => mockHttpClient.get(any())).thenAnswer(
        (_) async => http.Response(json.encode(mockHotelsJson), 200),
      );
      when(() => mockCacheService.cacheSearchResults(
      ...
      verify(() => mockCacheService.cacheSearchResults(
            ciudad: 'Medellín',
            pais: 'Colombia',
    ...
    test('falls back to cache on SocketException (Network Error)', () async {
      ...
      when(() => mockCacheService.getCachedSearchResults(
            ciudad: any(named: 'ciudad'),
            pais: any(named: 'pais'),
            fechaInicio: any(named: 'fechaInicio'),
            fechaFin: any(named: 'fechaFin'),
  ...
  group('SearchService - getLocationSuggestions', () {
    ...
    test('returns logic suggestions and merges them into cache on success', () async {
      ...
      when(() => mockCacheService.mergeDestinationSuggestions(any()))
          .thenAnswer((_) async => {});
      when(() => mockCacheService.getCachedDestinationSuggestions(any()))
          .thenAnswer((_) async => []);
      ...
      verify(() => mockCacheService.mergeDestinationSuggestions(any())).called(1);
    });


    test('falls back to cached suggestions on network failure', () async {
      final cachedSuggestions = [
        const LocationSuggestion(ciudad: 'MockCity', estadoProvincia: 'MockState', pais: 'MockCountry')
      ];
